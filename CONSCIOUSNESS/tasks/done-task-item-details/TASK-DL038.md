...
four cases the fix needs to distinguish. No automated end-to-end test exists for this (it would require
actually racing two EnterWorktree calls), so this is the strongest verification available short of that.
...
- A seat cannot introspect its own `--name` from inside the session (no env var carries it) — mitigated by instructing `ListAgents` first, whose self-identifying header line names the current session
